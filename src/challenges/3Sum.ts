...
const threeSumPointers = (nums: number[]): number[][] => {
  ...
  for (let i = 0; i < nums.length; i += 1) {
    // O(n)
    // If the current number is greater than 0, break from the loop
    ...
    while (left < right) {
      ...
      if (sum === 0) {
        ...
        // If the left pointer is equal to the number after it, increment the left pointer
        while (left < right && nums[left] === nums[left - 1]) {
          // O(1)
          left += 1
        }

        // If the right pointer is equal to the number before it, decrement the right pointer
